...
public class GALocal {
    ...
    public GALocal(int populationSize, double mutationRate, double crossoverRate, String kpFile, long seed) {
    ...
    //local search functions -- hill climbing:
    private double calculateTotal(int[] solution) {

        double totalValue = 0.0;
        Knapsack knapsack = population.get(0);

        for (int i = 0; i < solution.length; i++) {
            if (solution[i] == 1) { //checking if item is taken
    ...
    private int[] hillClimbing(int[] solution) { //where "solution" == route taken
        ...
        double currDist = calculateTotal(currSolution);
        ...
        do {
            ...
            for (int i = 0; i < currSolution.length - 1; i++) {
                for (int j = i + 1; j < currSolution.length; j++) {
                    ...
                    double newDistance = calculateTotal(currSolution);
    ...
    public void runGeneticAlgorithm(int maxGenerations) { //run the GA :)
        ...
        while (!checkStoppingCriteria(generationCount, maxGenerations)) {
            ...
            for (Knapsack knapsack : population) {
                int[] solution = knapsack.convertKnapsack(); //converting to solution format
                int[] improvedSolution = hillClimbing(solution); //local search
                knapsack.setItemsFromSolution(improvedSolution); //update knapsack items based on improved solution
            }
        ...
        System.out.println("GA + Local Search--> ");
