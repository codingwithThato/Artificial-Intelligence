...
class ils_copy_readwrite {
    ...
    public ils_copy_readwrite(int[][] costMatrix) {
        ...
        do {
            ...
            for (int i = 0; i < currSolution.length - 1; i++) {
                for (int j = i + 1; j < currSolution.length; j++) {
                    ...
                    int tmp = currSolution[i];//swapping two adj. campuses
    ...
    public void run() { 
        ...
        for (int i = 0; i < 1000; i++) { //1000 iterations
            content += iteration == MAX_ITERATIONS - 1 ? String.valueOf(bestDistance) : String.valueOf(bestDistance) + "\n";
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("ils_output.txt"))) {
